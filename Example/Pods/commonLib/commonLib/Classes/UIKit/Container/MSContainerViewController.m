...
- (void)viewDidLayoutSubviews
{
    ...
    CGFloat itemHeight=height -CGRectGetMaxY(_navigationView.frame);
    self.flowLayout.itemSize = CGSizeMake(width, itemHeight);
    self.collectionView.frame = CGRectMake(0, CGRectGetMaxY(_navigationView.frame)+self.topExtend, width, itemHeight);
}

-(UIView<MSNavigatorView> *)navigationView{
    if (!_navigationView) {
        typeof(self) __weak weakObj= self;
        _navigationView = [MSNavigationView navigationViewWithItems:nil itemClick:^(NSInteger selectedIndex) {
