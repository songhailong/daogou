...
- (void)setupView {
    ...
    CGFloat selfWidth=kBDKNotifyHUDDefaultWidth;
    
    CGSize size = [self.text sizeWithAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:16]}];
    if (size.width<150) {
        selfWidth=150;
    }else if(size.width>=150&&size.width<selfWidth){
        selfWidth=size.width;
    }
    
    
    self.frame=CGRectMake(0, 0, selfWidth, kBDKNotifyHUDDefaultHeight);
